...
export const BulkAssign = ({
  ...
  const rowValidator = (row: Record<string, any>): string | null => {
    if (typeof row?.categoryId !== "number" || row?.categoryId <= 0) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await ProductService.bulkAssign(formData);
      if (res) {
  ...
  return (
    <>
      <div className="flex justify-between">
        <a
          href="https://nology-app.s3.me-central-1.amazonaws.com/Bulk-Template/Assign-Category.xlsx"
          download
      ...
      <FileUploadValidator
        validationRules={{
          requiredHeaders: ["sku", "categoryId"],
          rowValidator,
        }}
        onValidData={(data) => {
